    ...
    on<_AddProductOrDetailToORder>((event, emit) async {
      ...
      if (detailProduct == null) {
        ...
        for (final FFFoodListsDefinition foodListDefinition in product?.foodListsDefinition ?? []) {
          ...
          for (final FFFoodListDefinitionDetail food in foodListDefinitionDetailSorted) {
            variations.add(FFVariation(
              selectionPriority: foodListDefinitionDetailSorted.indexOf(food) + 1,
              foodId: food.id,
              price: food.price,
              variationType: 1,
              foodName: food.name,
      ...
      if (order != null) {
        ...
        if (detailProduct != null && !productExist) {
          ...
          order.details.add(
            FFDetail(
              formatId: formatName != null ? product?.formatId : null,
              format: formatName,
              productId: productId,
        ...
        for (final FFDetail q in order.details) {
      ...
      //* Altrimenti creo un nuvo ordine e aggiungo il prodotto
        ...
        orderProducts.add(
          FFDetail(
            formatId: product?.formatName != null ? product?.formatId : null,
